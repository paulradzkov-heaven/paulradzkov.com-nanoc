...
def add_update_item_attributes
  ...
  items.each do |item|
    ...
    if item[:content_filename]
      ...
      item[:is_hidden] = true if item[:content_filename] =~ /assets\// || ext == '.xml' || ext == '.txt' || ext == '.png' || ext == '.jpg' || ext == '.jpeg'
    end

    if item[:kind] == "article"
      # filename might contain the created_at date
      item[:created_at] ||= derive_created_at(item)
      # sometimes Nanoc stores created_at as Date instead of String causing a bunch of issues
